...
function verifyDeviceConnection(dt, ct, id, cb) {
	...
	function handleGetHandleInfo(handleInfo) {
		...
		ljm.LJM_Close.async(handleInfo.H, handleDeviceClose);
	}
	function handleDeviceOpen(openInfo) {
		...
		if(openInfo.ljmError === 0) {
			deviceOpened = true;
			// 
			getHandleInfo(openInfo.handle, handleGetHandleInfo);
		} else {
			ljm.LJM_Close.async(openInfo.handle, handleDeviceClose);
		}
	}
	ljm.LJM_Open.async(
		dt,
